...
class Bot:
    def __init__(self, headless=False):
        # Tạo đối tượng Options cho trình duyệt Edge
        self.edge_options = EdgeOptions()
        self.edge_options.use_chromium = True
        self.edge_options.headless = headless  # Đặt tùy chọn headless

        # Khởi tạo WebDriver với trình duyệt Edge, đường dẫn và các tùy chọn đã thiết lập
        self.driver = Edge(executable_path=r"D:\\QT\\77_12_2023\\msedgedriver.exe", options=self.edge_options)
        
        # Chờ implcitly 5 giây
    ...
    def groupScraper(self, url):
        ...
        try:
            self.driver.get(url + '?')
            members_btn = self.driver.find_element_by_xpath('//a[text()="Thành viên"]')
            members_btn.click()
